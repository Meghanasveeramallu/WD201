...
app.get("/todos",
  ...
  async function (request, response) {
    try {
      const isloggedin = request.user.id;
      const itemsOverDue = await Todo.itemsOverDue(isloggedin);
      const itemsDueToday = await Todo.itemsDueToday(isloggedin);
      const itemsDueLater = await Todo.itemsDueLater(isloggedin);
      const itemsCompleted = await Todo.completedItemsAre(isloggedin);
      if (request.accepts("html")) {
        ...
        response.json({
          itemsOverDue,itemsDueToday,itemsDueLater,itemsCompleted,
        });
      }
    } catch (err1) {
      console.log(err1);
...
app.post("/users", async (request, response) => {
  ...
  if (request.body.password < 8) {
    request.flash("error", "Length of the password should be atleast 8");
    return response.redirect("/signup");
  }
...
app.post("/todos",
  ...
  async function (request, response) {
    if (request.body.title.length < 5) {
      request.flash("error", "Lenght of the title should be atleast 5");
...
app.delete("/todos/:id",
  ...
  async function (req, resp) {
    console.log("Delete a To Do Item with a particular id : ", req.params.id);
